...
begin
  ...
  comp := TJSObject.new;
  comp['template'] :=
    '  <!-- Main wrapper for the icon using inline-flex for alignment -->' +
    '  <span class="bi-icon-wrapper" :style="''display: inline-flex; width: '' + size + ''px; height: '' + size + ''px;''">' +
    '    ' +
    '    <!-- Render specific SVG based on the ''name'' property -->' +
    '    <!-- We use direct V-IF here to avoid the need for external sprite files which are often blocked in local environments -->' +
    '    ' +
    '    <svg v-if="name == ''bootstrap-fill''" xmlns="http://www.w3.org/2000/svg" :width="size" :height="size" fill="currentColor" viewBox="0 0 16 16">' +
