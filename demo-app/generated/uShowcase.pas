...
begin
  ...
  comp['template'] :=
    ...
    '      <p>Current Font: <strong>Inter & Outfit</strong> (100% Local)</p>' +
    '    </div>' +
    '' +
    '    <!-- Icon Gallery Demonstration -->' +
    '    <!-- Icons are rendered using the ''b-icon'' component with explicit SVG templates -->' +
    '    <div class="section-card">' +
    '      <h3>Icon Gallery (Inline Local SVGs)</h3>' +
    '      <div class="icon-grid">' +
    '        <div class="icon-box">' +
    ...
    '    <!-- Dynamically switches global CSS files from the local assets folder -->' +
    '    <div class="section-card">' +
    '      <h3>Theme Selector (Dynamic CSS)</h3>' +
    '      <p class="text-muted">Click to swap the main Bootstrap CSS file in real-time.</p>' +
    '      <div class="theme-buttons d-flex gap-2 flex-wrap">' +
    '        <button class="btn btn-outline-secondary" @click="handleTheme(''default'')">Default</button>' +
    ...
    '      </div>' +
    '      <div class="mt-3">' +
    '         Selected Theme: <span class="badge bg-secondary">{{ currentThemeName }}</span>' +
    '      </div>' +
    '    </div>' +
    '  </div>';
